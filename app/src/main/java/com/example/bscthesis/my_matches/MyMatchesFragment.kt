...
class MyMatchesFragment: Fragment() {
    ...
    override fun onCreateView(
        ...
        userListenerRegistration = FirestoreUtil.getCurrentUserContacts(this.requireActivity(),this::updateRecycleView)
    ...
    private fun updateRecycleView(items: List<BindableItem<DogsItemBinding>>){
        fun init(){
            binding.recyclerView.apply {
                layoutManager = LinearLayoutManager(this@MyMatchesFragment.context)
    ...
    private val onClickItem = OnItemClickListener {item, view ->
        if (item is DogItem){
            ...
            findNavController().navigate(
                MyMatchesFragmentDirections.actionMyMatchesToMessage())
            shouldInitRecyclerView = true
        }
    }
}